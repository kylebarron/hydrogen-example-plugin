...
const onResultsCallbackExample = (next, code, onResults) => {
  ...
  next.execute(code, (message, channel) => {
    ...
    if (message.header && message.header.msg_type === "execute_input") {
      atom.notifications.addSuccess(
        `Your input count is: ${message.content.execution_count}`
      );
    }
...
export const kernelMiddlewareExample = {
  execute: onResultsCallbackExample
  // execute just one middleware method:
